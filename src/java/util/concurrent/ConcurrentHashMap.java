 ...
 * <p>支持检索的完全并发和更新的高预期并发的哈希表</p>
 * This class obeys the
 * same functional specification as {@link java.util.Hashtable}, and
 * includes versions of methods corresponding to each method of
 * {@code Hashtable}.
 * <p>此类遵循与 Hashtable 相同的功能规范，并包含与 Hashtable 的每个方法对应的方法版本。</p>
 * However, even though all operations are
 * thread-safe, retrieval operations do <em>not</em> entail locking,
 * and there is <em>not</em> any support for locking the entire table
 * in a way that prevents all access.
 * <p>但是，即使所有操作都是线程安全的，检索操作也不需要锁定，并且不支持以阻止所有访问的方式锁定整个表</p>
 * This class is fully
 * interoperable with {@code Hashtable} in programs that rely on its
 * thread safety but not on its synchronization details.
 * <p>在依赖线程安全但不依赖同步细节的程序中，此类与HashTable完全可互操作</p>
 *
 * <p>Retrieval operations (including {@code get}) generally do not
 * block, so may overlap with update operations (including {@code put}
 * and {@code remove}).
 * <p>检索操作包括get一般不会阻塞，因此可能与更新操作（包括put和remove）重叠</p>
 * Retrievals reflect the results of the most
 * recently <em>completed</em> update operations holding upon their
 * onset. (More formally, an update operation for a given key bears a
 * <em>happens-before</em> relation with any (non-null) retrieval for
 * that key reporting the updated value.)
 * <p>检索反映了最近完成的更新操作在其开始时保持的结果。更正式地说，给定键的更新操作
 * 与报告更新值的该键的任何非空检索具有发生前的关系</p>
 *
 * For aggregate operations
 * such as {@code putAll} and {@code clear}, concurrent retrievals may
 * reflect insertion or removal of only some entries.
 * <p>对于putAll和clear等聚合操作，并发检索可能反映插入或者删除只有一些条目。也就是说clear方法没有
 * 使用全局锁锁住整个table数组（或者jdk7中的整个Segment数组，每一个segment 就是一个map）这就clear会针对table数组
 * 中的每个链表进行清除，因此在clear的时候 get能够读取到部分数据，同时put也能放入数据。
 * </p>
 * Similarly,
 * Iterators, Spliterators and Enumerations return elements reflecting the
 * state of the hash table at some point at or since the creation of the
 * iterator/enumeration.
 * <p>类似的，迭代器，拆分器和枚举返回反映哈希表在创建迭代器/枚举时或之后的某个时间点的状态的元素。</p>
 *
 * They do <em>not</em> throw {@link
 * java.util.ConcurrentModificationException ConcurrentModificationException}.
 * <p>他们不会抛出 并发修改异常</p>
 * However, iterators are designed to be used by only one thread at a time.
 * <p>但是迭代器设计为一次只能由一个线程使用。</p>
 * Bear in mind that the results of aggregate status methods including
 * {@code size}, {@code isEmpty}, and {@code containsValue} are typically
 * useful only when a map is not undergoing concurrent updates in other threads.
 * <p>请记住，包括size、isEmpty 和containsvalue在内的聚合状态方法的结果通常仅在map未在其他线程中进行并发更新时才有用。否则这些方法
 * 的结果反映的瞬态状态可能足以用于监控或估计目的，但不适用于程序控制。
 *
 * </p>
